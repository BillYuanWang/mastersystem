...
function renderStudents() {
  ...
  records.forEach((student) => els.studentBoard.append(createStudentColumn(student)));
}
...
function createStudentColumn(student) {
  const column = document.createElement("article");
  column.className = "student-column";

  const head = document.createElement("div");
  head.className = "student-column-head";
  const name = document.createElement("h3");
  name.textContent = student.name;
  const summary = document.createElement("p");
  const counts = studentEnrollmentCounts(student.items);
  summary.textContent = `${student.items.length} 门课 · 按期 ${counts.term} · 按N次 ${counts.passes}`;
  head.append(name, summary);

  const amount = document.createElement("div");
  amount.className = "student-total";
  amount.textContent = studentPaymentSummary(student.items);
  ...
  column.append(head, amount, list);
  return column;
}
...
function createStudentCourseBlock(studentName, item) {
  ...
  const button = document.createElement("button");
  button.type = "button";
  button.className = `student-course-block ${enrollment.type}`;
  button.title = studentCourseHoverText(course, enrollment);
  button.addEventListener("click", () => openReadonlyCourseDialog(course.id, studentName, enrollment.type));

  const line = document.createElement("span");
  line.className = "student-course-line";
  line.textContent = `${dayLabel(course.day)} · ${course.name} · ${enrollmentTypeLabel(enrollment.type)}`;
  const meta = document.createElement("span");
  meta.className = "student-course-meta";
  meta.textContent = [course.start, roomLabel(course.room), course.teacher].filter(Boolean).join(" · ");
  ...
  button.append(line, meta, money, hover);
  return button;
}
